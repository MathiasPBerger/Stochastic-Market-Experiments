...
The analyses relies on enveloppe theorems developed for constrained convex programming problems \cite{Milgrom2002}. In essence, these theorems state that the derivative of the value function can be simply obtained as the derivative of the Lagrangian function evaluated at an optimal solution.

Let $V^*: \mathbb{R} \times \mathbb{R}_+ \rightarrow \mathbb{R}$ be the value function of the stochastic market clearing problem, with $V^*(\mu, \sigma)$ the value associated with a specific pair of parameters $(\mu, \sigma)$. In addition, let $(p_k^*, \alpha_k^*)_{\forall k}$ and $(\lambda^*, \chi^*)$ represent optimal primal and dual variables, one obtains
\begin{align*} 
...
\Leftrightarrow \mu \frac{\partial V^*}{\partial \mu} &= \sum_k\alpha_k^*\Big(\chi^* - \Phi^{-1}(1-\epsilon_k) \sigma(\underline{\nu}_k^*+\overline{\nu}_k^*) - 2c_k^Q \alpha_k^* \sigma^2\Big).
\end{align*}
Then, we have
\begin{align*}
...
&= \sum_k\alpha_k^*\big(2c_k^Q\alpha_k^*\sigma + \Phi^{-1}(1-\epsilon_k) (\underline{\nu}_k^* + \overline{\nu}_k)^*\big)\\
&=\sum_k\frac{\alpha_k^*}{\sigma}\Big(2c_k^Q\alpha_k^*\sigma^2 + \Phi^{-1}(1-\epsilon_k) \sigma (\underline{\nu}_k^* + \overline{\nu}_k^*)\Big)\\
\Leftrightarrow \sigma \frac{\partial V^*}{\partial \sigma} &= \sum_k \alpha_k^*\Big(2c_k^Q\alpha_k^*\sigma^2 + \Phi^{-1}(1-\epsilon_k) \sigma (\underline{\nu}_k^* + \overline{\nu}_k^*)\Big).
\end{align*}
...
\end{equation*}
In other words, the reserve price measures the sensitivity of the expected cost with the respect to the mean and standard deviation.
\section{Comments}
