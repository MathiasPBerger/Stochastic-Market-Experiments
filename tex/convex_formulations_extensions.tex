...
\end{equation*}
for brevity. Note that $\underline{\nu}_k^* \ge 0$, $\overline{\nu}_k^* \ge 0$, $\forall k$, and $||\alpha_k^*||_{\Sigma} \ge 0$. In addition, $c_k^Q > 0, \forall k,$ and $\phi_k \ge 0, \forall k$, by assumption (since $0 < \epsilon_k \le 0.5, \forall k,$ for a normal distribution and $\phi_k$ is always positive in the distributionally robust setting). Thus, $\varphi_k \ge 0, \forall k$. By virtue of Proposition \ref{reserveallocation}, $\alpha_k = \eta_k e$ for some $\eta_k \in [0, 1]$, which implies that $\alpha_{ki} = \alpha_{kj} = \eta_k$ for all $i$ and $j$. The sensitivity with respect to $\sigma_i$ can therefore be re-written as
\begin{equation*}
