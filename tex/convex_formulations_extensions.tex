...
If no reformulated chance constraints are tight at the optimum, reserve allocations $\alpha_k \in [0, 1]^N$ are symmetric in the sense that
\begin{equation*}
\alpha_{k}^* = \eta_k e,
\end{equation*}
...
\end{proposition}
\begin{proof}
The proof proceeds in two parts. We start by showing that
\begin{equation*}
\alpha_k^* = \frac{1}{2 c_k^Q} \Sigma^{-1} \Big(\lambda^* \mu + \chi^* \Big).
\end{equation*}
Next, we show that the vector expression on the right-hand side above is a multiple of the all-ones vector, that is,
\begin{equation*}
\Sigma^{-1} \Big(\lambda^* \mu + \chi^* \Big) = \gamma_k e,
\end{equation*}
for some appropriate $\gamma_k$, from which the result is then derived.
