...
\begin{lemma} (Reserve Allocation)
If no reformulated chance constraints are tight at the optimum, reserve allocations $\alpha_k^* \in [0, 1]^N$ can be obtained as
\begin{equation*}
\alpha_k^* = \frac{1}{2 c_k^Q} \Sigma^{-1} \Big(\lambda^* \mu + \chi^* \Big).
\end{equation*}
\end{lemma}
\begin{proof}
First, note that from complementary slackness, $\overline{\mu}_k^* = \underline{\mu}_k^* = 0$ if chance constraints are not tight at the optimum. Next, let us write the stationary conditions of the Lagrangian with respect to $p_k$ and $\alpha_k$. In vector form, they read
\begin{align*}
...
\end{align*}
where terms associated with $\overline{\mu}_k^*$ and $\underline{\mu}_k^*$ do not appear since they are both equal to zero. Noting that $(\alpha_k^\top \mu) \mu = (\mu \mu^\top) \alpha_k$ and re-arranging terms yields
\begin{align*}
