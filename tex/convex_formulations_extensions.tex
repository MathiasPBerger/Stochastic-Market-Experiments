...
\end{proposition}
\begin{proof}
\textcolor{orange}{The proof must be completed}
...
f(x, y) =& x \big(2c_k^Q(p_k^* - (\alpha_k^*)^\top \mu)\big) - c_k^Q(x - y^\top \mu)^2 + \overline{\nu}_k^* \overline{p}_k - \Phi_k^{-1} ||y||_\Sigma (\underline{\nu}_k^* + \overline{\nu}_k^*)\\ 
    &+ y^T \Big(2c_k^Q\big(M \alpha_k^* - p_k^* \mu + \Sigma \alpha_k^*\big) + \frac{\Phi_k^{-1}}{||\alpha_k^*||_\Sigma}(\underline{\nu}_k^* +\overline{\nu}_k^*) \Sigma \alpha_k^*\Big) - c_k^Q y^T \Sigma y,
\end{align*}
...
\frac{\partial^2 f}{\partial x \partial y} &= 2 c_k^Q \mu,\\
\frac{\partial^2 f}{\partial y \partial x} &= 2 c_k^Q \mu^\top,\\
...
\end{align*}
We note that $-\partial^2 f/\partial y^2$ is positive definite. Indeed, $M$ is a positive semi-definite rank-one matrix, the matrix in parentheses is positive semi-definite (\textcolor{orange}{since it is the Hessian of a weighted 2-norm, which is convex}) and its coefficients are non-negative, $c_k^Q > 0$ and $\Sigma$ is positive definite. Then, \textcolor{orange}{it can be shown that $-H$ is positive definite}, such that $H$ is negative definite and $(x, y) = (p_k^*, \alpha_k^*)$ is a maximum of $f$. 

In other words, $f(x, y) \le f(p_k^*, \alpha_k^*), \forall (x, y),$ and in particular $ f(p_k^\star, \alpha_k^\star) \le f(p_k^*, \alpha_k^*)$. It then follows that
\begin{equation*}
