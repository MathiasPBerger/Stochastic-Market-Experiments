...
\end{align*}
Note that $\underline{\nu}_k^* \ge 0$, $\overline{\nu}_k^* \ge 0$, $\forall k$, $||\alpha_k^*||_{\Sigma} \ge 0$. In addition, $c_k^Q > 0, \forall k,$ and $0 < \epsilon_k \le 0.5, \forall k,$ by assumption, which implies that $\Phi^{-1}_k \ge 0, \forall k$. Thus, the second term in parentheses is non-negative. A sufficient condition for the sensitivity to be non-negative is therefore that
\begin{equation*}
v(\rho) = \alpha_{ki}^* \Big(\sigma_i \alpha_{ki}^* + \sum_{j \ne i} \rho_{ij} \sigma_j\alpha_{kj}^*\Big) \ge 0, \forall k,
\end{equation*}
with $v: \mathcal{K} \rightarrow \mathbb{R}$ and $\mathcal{K} = \{A \in \mathbb{R}^{N \times N}: -1 \le A_{ij} \le 1, i \ne j, A_{ii} = 1, A \succeq 0\}$ the set of $N \times N$ correlation matrices. We use this condition to prove the first two results:
\begin{itemize}
\item[(i)] If forecast errors are independent, $\rho_{ij} = 0, i \ne j$. Hence, we have
\begin{equation*}
v(\rho) = \sigma_i (\alpha_{ki}^*)^2 \ge 0,
\end{equation*}
since $\sigma_i > 0$ and $\alpha_{ki}^* \ge 0$.
\item[(ii)] If forecast errors are positively-correlated, $\rho_{ij} \ge 0, i \ne j$. It follows that $v(\rho) \ge 0$.
