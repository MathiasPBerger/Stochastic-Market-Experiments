...
\begin{align*} 
\frac{\partial V^*}{\partial \mu_i} =& \frac{\partial \mathcal{L}^*}{\partial \mu_i}\\
=& \sum_k \Big(-c_k^L \alpha_{ki}^* + 2 c_k \big((\alpha_{ki}^*)^2 \mu_i - p_k^* \alpha_{ki}^*\big) + \underline{\nu}_k^* \alpha_{ki}^* - \overline{\nu}_k^* \alpha_{ki}^*\Big)\\
=& \sum_k \alpha_{ki}^* \Big(-c_k^L + 2 c_k (\alpha_{ki}^* \mu_i - p_k^*) + \underline{\nu}_k^* - \overline{\nu}_k^*\big)\\
...
\end{align*}
where the first line follows from Lemma \ref{lemma1}, the second line is obtained by taking the partial derivative of the Lagrangian function with respect to $\mu_i$ and evaluating it at the optimal solution, re-arranging terms yields the third line, the fourth line is obtained by substituting the stationarity condition of the Lagrangian function with respect to $\alpha_{ki}$ (satisfied at optimality), while re-arranging terms gives the fifth lines. It immediately follows that
\begin{align*}
...
\end{equation*}
\end{proof}
We also have the following result - \textcolor{red}{not sure that it always holds in the general set-up - this will probably depend on the correlation between forecast errors}.
\begin{proposition}
\textcolor{red}{The expected system cost increases with the standard deviation of the forecast error.}
\end{proposition}
\begin{proof}
It follows from the proof of the previous proposition that
\begin{align*}
\frac{\partial V^*}{\partial \sigma_i} =& \sum_k\frac{\alpha_{ki}^*}{\sigma_i}\Big(2c_k^Q\big(\sigma_i^2 \alpha_{ki}^* + \sum_{j \ne i} \rho_{ij} \sigma_i \sigma_j\alpha_{kj}^*\big)\\
&+ \Phi_k^{-1} \frac{\sigma_i^2 \alpha_{ki}^* + \sum_{j \ne i} \rho_{ij} \sigma_i \sigma_j \alpha_{kj}^*}{||\alpha_k^*||_{\Sigma}}(\underline{\nu}_k^* + \overline{\nu}_k^*)\big)\Big)\\
=& \frac{1}{\sigma_i} \sum_k\big(\sigma_i^2 (\alpha_{ki}^*)^2 + \sum_{j \ne i} \rho_{ij} \sigma_i \sigma_j\alpha_{ki}^* \alpha_{kj}^*\big)\Big(2c_k^Q + \Phi_k^{-1} \frac{1}{||\alpha_k^*||_{\Sigma}}(\underline{\nu}_k^* + \overline{\nu}_k^*)\big)\Big).
\end{align*}
Note that $\alpha_{ki}^* \ge 0, \forall i$, $\underline{\nu}_k^* \ge 0$, $\overline{\nu}_k^* \ge 0$, $\forall k$. By definition, $\sigma_i > 0, \forall i$, $c_k^Q \ge 0$ and $\epsilon_k \le 0.5$, which implies that $\Phi^{-1}_k \ge 0$. However, $-1 \le \rho_{ij} \le 1$, such that the first term in parentheses may be negative. Hence, the key problem is then to identify conditions under which $\sigma_i^2 (\alpha_{ki}^*)^2 + \sum_{j \ne i} \rho_{ij} \sigma_i \sigma_j \alpha_{ki}^* \alpha_{kj}^* \ge 0 $.
\end{proof}
...
\frac{\partial \mathcal{L}_k}{\partial \alpha_{ki}} =& - c_k^L \mu_i + 2 c_k^Q\Big(\alpha_{ki}\mu_i^2 - p_k \mu_i + \sigma_i^2 \alpha_{ki} + \sum_{j \ne i} \kappa_{ij} \alpha_{kj}\Big) - \chi_i \\
...
\frac{\partial \mathcal{L}}{\partial \alpha_{ki}} =& - c_k^L \mu_i + 2 c_k^Q\Big(\alpha_{ki} \mu_i^2 - p_k \mu_i + \sigma_i^2 \alpha_{ki} + \sum_{j \ne i} \kappa_{ij} \alpha_{kj}\Big)\\
&+ \underline{\nu}_k \Big(\mu_i + \Phi_k^{-1} \frac{\sigma_i^2 \alpha_{ki} + \sum_{j \ne i} \kappa_{ij} \alpha_{kj}}{||\alpha_k||_{\Sigma}}\Big)\\
