...
Before analysing the sensitivity of the expected system cost with respect to the standard deviation of the forecast error, we need a result pertaining to the way that reserve is allocated across wind farms for a given generator.
...
\begin{proposition} (Symmetry of Reserve Allocation)
For each generator $k$, reserves $\alpha_k^* \in [0, 1]^N$ are allocated symmetrically across wind farms, that is,
\begin{equation*}
\alpha_{k}^* = \eta_k e,
\end{equation*}
for some $\eta_k \in [0, 1]$ such that $\sum_k \eta_k = 1$, and where $e \in \mathbb{R}^N$ denotes the all-ones vector.
...
\end{align*}
Then, in vector form, the stationarity conditions of the updated Lagrangian with respect to $p_k$ and $\alpha_k$ read
\begin{align*}
...
To obtain the second intermediate result, note that reserves must be allocated so that the forecast error from each wind farm is fully covered. Thus, we have that
\begin{equation*}
\sum_k \alpha_k = e.
\end{equation*}
...
\end{align*}
Note that $\underline{\nu}_k^* \ge 0$, $\overline{\nu}_k^* \ge 0$, $\forall k$, $||\alpha_k^*||_{\Sigma} \ge 0$. In addition, $c_k^Q > 0, \forall k,$ and $0 < \epsilon_k \le 0.5, \forall k,$ by assumption, which implies that $\phi_k \ge 0, \forall k$. Thus, the second term in parentheses is non-negative. A sufficient condition for the sensitivity with respect to $\sigma_i$ to be non-negative is therefore that
\begin{equation*}
...
\end{align*}
Note that $\underline{\nu}_k^* \ge 0$, $\overline{\nu}_k^* \ge 0$, $\forall k$. Clearly, $||\alpha_k^*||_{\Sigma} \ge 0$. By assumption, $c_k^Q > 0, \forall k$, and $0 < \epsilon_k \le 0.5, \forall k$, which implies that $\phi_k \ge 0, \forall k$. Hence, each term in the sum is non-negative and the result follows.
\end{proof}
