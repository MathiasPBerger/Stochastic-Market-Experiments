...
@constraint(model, aff_expr_slack_max[g = 1:n_g], s_max[g] == p_max[g] - p[g])
@constraint(model, aff_expr_slack_min[g = 1:n_g], s_min[g] == p[g] - p_min[g])
...
true_generator_cost = [(true_C_Q[g]*(p_scheduled[g]^2 + alpha_scheduled[g,:]'*cov_mat*alpha_scheduled[g,:]) + true_C_L[g]*p_scheduled[g] ) for g = 1:n_g];
reported_generator_cost = [(C_Q[g]*(p_scheduled[g]^2 + alpha_scheduled[g,:]'*cov_mat*alpha_scheduled[g,:]) + C_L[g]*p_scheduled[g]) for g = 1:n_g];
...
std_sensitivity_coeffs, std_sensitivity = zeros(n_w, n_g), zeros(n_w);
mean_sensitivity_coeffs, mean_sensitivity = zeros(n_w, n_g), zeros(n_w);
...
for k = 1:n_g
    estimated_generator_profit[k] = C_Q[k] * (p_scheduled[k]^2 + alpha_scheduled[k,:]'* cov_mat * alpha_scheduled[k, :]) + dual_max_prod[k] * p_max[k]
end
