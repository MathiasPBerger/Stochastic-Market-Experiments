...
@constraint(model, max_prod[g = 1:n_g], kappa^2 * alpha[g,:]'*cov_mat*alpha[g,:] <= s_max[g]^2)
@constraint(model, min_prod[g = 1:n_g], kappa^2 * alpha[g,:]'*cov_mat*alpha[g,:] <= s_min[g]^2)
...
for i = 1:n_w
    for k = 1:n_g
        ...
        std_sensitivity[i] += std_sensitivity_coeffs[i, k] * (2 * C_Q[k] + (kappa / sqrt(alpha_scheduled[k,:]'*cov_mat*alpha_scheduled[k,:])) * (dual_min_prod[k] + dual_max_prod[k]))
